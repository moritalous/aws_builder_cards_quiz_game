...
// Set event emitter for client-server communication
bedrockClient.setEventEmitter(eventEmitter);

// Listen for events from the Bedrock client
eventEmitter.on("requestPhotoCapture", () => {
  console.log("Photo capture requested by AI");
  // Broadcast to all connected clients
  io.emit("takePhotoRequest");
});
...
io.on("connection", (socket) => {
  ...
  try {
    ...
    // Option to automatically take a photo each time the user starts voice input
    socket.on("audioStart", async (data) => {
      try {
        console.log("Audio start received", data);
        await session.setupStartAudio();

        // Send automatic photo capture request (optional feature)
        // io.to(socket.id).emit('takePhotoRequest'); // Uncomment to enable
      } catch (error) {
    ...
    // Event handler for receiving photo data
    socket.on("capturedPhoto", async (photoDataUrl) => {
      try {
        console.log("Received photo from client");
        // Pass photo data to Bedrock client
        bedrockClient.setLatestCapturedImage(photoDataUrl);
      } catch (error) {
    ...
    // Receive auto capture settings
    socket.on("setAutoCapture", (enabled) => {
      try {
        console.log(`Setting auto capture to: ${enabled}`);
        bedrockClient.toggleAutoCapture(enabled);
      } catch (error) {
        console.error("Error setting auto capture:", error);
        socket.emit("error", {
          message: "Error setting auto capture",
          details: error instanceof Error ? error.message : String(error),
        });
      }
    });
